            ...
            <div class="row justify-content-center">
                <div class="col-lg-10">
                    ...
                    <form action="{{ route('superadmin.settings.payment.update') }}" method="POST"
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <select name="default_currency"
                                            ...
                                            <option value="BDT"
                                                {{ old('default_currency', $settings['default_currency'] ?? 'BDT') == 'BDT' ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="currency_symbol_position" class="form-select">
                                            <option value="before"
                                                {{ old('currency_symbol_position', $settings['currency_symbol_position'] ?? 'before') == 'before' ? 'selected' : '' }}>
                                                Before (৳100)</option>
                                            <option value="after"
                                                {{ old('currency_symbol_position', $settings['currency_symbol_position'] ?? '') == 'after' ? 'selected' : '' }}>
                                                After (100৳)</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-12">
                                        <label class="form-check form-switch">
                                            ...
                                            <input class="form-check-input" type="checkbox" name="sslcommerz_enabled"
                                                value="1"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="password" name="sslcommerz_store_password"
                                            ...
                                            value="{{ old('sslcommerz_store_password') }}"
                                            placeholder="{{ !empty($settings['sslcommerz_store_password']) ? '••••••••  (already set — leave blank to keep)' : 'Enter store password' }}">
                                        @error('sslcommerz_store_password')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        @if (!empty($settings['sslcommerz_store_password']))
                                            <small class="text-success"><i class="ti ti-check me-1"></i>Password is
                                                currently saved. Leave blank to keep it unchanged.</small>
                                        @else
                                            <small class="text-danger"><i class="ti ti-alert-circle me-1"></i>No password
                                                saved yet. Please enter your SSLCommerz store password.</small>
                                        @endif
                                    </div>
                                    <div class="col-md-12">
                                        ...
                                        <select name="sslcommerz_environment" class="form-select">
                                            <option value="sandbox"
                                                {{ old('sslcommerz_environment', $settings['sslcommerz_environment'] ?? 'sandbox') == 'sandbox' ? 'selected' : '' }}>
                                                Sandbox (Test)</option>
                                            <option value="live"
                                                {{ old('sslcommerz_environment', $settings['sslcommerz_environment'] ?? '') == 'live' ? 'selected' : '' }}>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="bkash_app_key"
                                            class="form-control @error('bkash_app_key') is-invalid @enderror"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="password" name="bkash_app_secret"
                                            class="form-control @error('bkash_app_secret') is-invalid @enderror"
                                            value="{{ old('bkash_app_secret') }}"
                        ...
                        <div class="mb-4 card">
                            <div class="card-header">
                                <h3 class="card-title"><i class="ti ti-phone me-2 text-orange"></i> Nagad Configuration
                                </h3>
                            </div>
                            <div class="card-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="nagad_merchant_id"
                                            class="form-control @error('nagad_merchant_id') is-invalid @enderror"
                                            value="{{ old('nagad_merchant_id', $settings['nagad_merchant_id'] ?? '') }}"
                                            placeholder="merchant_id">
                                        @error('nagad_merchant_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Merchant Number</label>
                                        <input type="text" name="nagad_merchant_number"
                                            class="form-control @error('nagad_merchant_number') is-invalid @enderror"
                                            value="{{ old('nagad_merchant_number', $settings['nagad_merchant_number'] ?? '') }}"
                                            placeholder="01XXXXXXXXX">
                                        @error('nagad_merchant_number')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // NOTE: Flash toasts (success/error) are handled globally by
                // partials/alerts.blade.php — no need to duplicate them here.

                // ==========================================
                // FORM VALIDATION (Fixed Checkbox Targeting)
                // ==========================================
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    if (document.querySelector('input[type="checkbox"][name="sslcommerz_enabled"]')?.checked)
                        enabledGateways.push('SSLCommerz');
                    if (document.querySelector('input[type="checkbox"][name="bkash_enabled"]')?.checked)
                        enabledGateways.push('bKash');
                    if (document.querySelector('input[type="checkbox"][name="nagad_enabled"]')?.checked)
                        enabledGateways.push('Nagad');
                    if (document.querySelector('input[type="checkbox"][name="stripe_enabled"]')?.checked)
                        enabledGateways.push('Stripe');
                    if (document.querySelector('input[type="checkbox"][name="paypal_enabled"]')?.checked)
                        enabledGateways.push('PayPal');

                    if (enabledGateways.length === 0) {
                        e.preventDefault(); // Stop form submission

                        if (window.showSmartToast) {
                            window.showSmartToast('warning', 'Please enable at least one payment gateway',
                                'Validation');
                        }
                    }
                });
