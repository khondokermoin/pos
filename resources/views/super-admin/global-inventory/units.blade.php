...
@section('title', 'Global Units (UOM)')
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">
                    <h4 class="mb-0">Global Units of Measure</h4>
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('superadmin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active">Global Units</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <ul class="nav nav-tabs mb-4">
            ...
            <li class="nav-item"><a class="nav-link active" href="{{ route('superadmin.global-units.index') }}"><i
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0"><i class="ti ti-plus me-2"></i>Add Unit</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('superadmin.global-units.store') }}">
                            @csrf
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="short_code"
                                    ...
                                    value="{{ old('short_code') }}" placeholder="e.g. Kg" required>
                                @error('short_code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0"><i class="ti ti-ruler me-2 text-primary"></i>All Global Units <span
                                class="badge bg-secondary ms-2">{{ $units->total() }}</span></h5>
                    </div>
                                ...
                                <tbody>
                                    @forelse($units as $unit)
                                        <tr>
                                            ...
                                            <td><span
                                                    class="badge bg-primary-subtle text-primary fs-13">{{ $unit->short_code }}</span>
                                            </td>
                                            <td>
                                                @if ($unit->is_active)
                                                    <span
                                            ...
                                            <td class="text-end">
                                                <button type="button" class="btn btn-sm btn-warning me-1"
                                                    data-bs-toggle="modal" data-bs-target="#editUnit{{ $unit->id }}"><i
                                                        class="ti ti-edit"></i></button>
                                                <form method="POST"
                                                    action="{{ route('superadmin.global-units.destroy', $unit) }}"
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">Edit Unit</h5><button type="button"
                                                            class="btn-close" data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <form method="POST"
                                                        action="{{ route('superadmin.global-units.update', $unit) }}">
                                                        @csrf @method('PUT')
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mb-3"><label class="form-label">Short
                                                                    Code</label><input type="text" name="short_code"
                                                                    class="form-control" value="{{ $unit->short_code }}"
                                                                    required></div>
                                                            <div class="form-check form-switch"><input
                                                                    class="form-check-input" type="checkbox"
                                                                    name="is_active" value="1"
                                                                    {{ $unit->is_active ? 'checked' : '' }}><label
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-4 text-muted"><i
                                                    class="ti ti-ruler-off fs-24 d-block mb-2"></i>No global units yet.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
