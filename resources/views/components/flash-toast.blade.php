@props(['type' => 'info', 'title' => null, 'message' => '', 'duration' => 5000])
...
<div class="flash-toast-item" data-duration="{{ $duration }}"
    ...
    <div style="display:flex;align-items:flex-start;gap:12px;">
        ...
        <div style="flex:1;min-width:0;">
            <p style="margin:0 0 2px 0;font-size:13px;font-weight:600;color:{{ $s['title'] }};line-height:1.4;">
                {{ $finalTitle }}</p>
            <p style="margin:0;font-size:13px;color:{{ $s['text'] }};line-height:1.5;word-break:break-word;">
                {{ $message }}</p>
        </div>
        ...
        <button
            ...
            style="flex-shrink:0;background:none;border:none;cursor:pointer;padding:2px;color:{{ $s['text'] }};opacity:0.6;line-height:1;margin-top:-2px;"
            aria-label="Close">
    ...
    {{-- Progress bar --}}
    <div style="margin-top:10px;height:3px;border-radius:2px;background:{{ $s['border'] }};overflow:hidden;">
...
<script>
    (function() {
        // এই নির্দিষ্ট toast-এর progress bar চালু করো
        document.addEventListener('DOMContentLoaded', function() {
            var toasts = document.querySelectorAll('.flash-toast-item[data-duration]');
            toasts.forEach(function(item) {
                if (item.dataset.progressStarted) return;
                item.dataset.progressStarted = '1';
                var dur = parseInt(item.dataset.duration) || 5000;
